$(function(){
	...
	$(document).ready(function() {		
		...
		if($("#tenId-pk").val()!=''){
			 $("#sel3").removeAttr('readonly');
			
		}
		else{
			$("#sel3").val("A");
			$("#sel3").prop("readonly", true);
		}
	  ...
	  var currTable = $('#tenTbl').DataTable( {
			...
			"columns": [
				...
				{ 
					"data": "tenId",
					"render": function ( data, type, full, meta ) {
						return '<form action="editClientForm" method="post"><input type="hidden" name="id" value='+full.tenId+'><input type="submit"  class="hyperlink-btn" value="Edit" ></form>';
					}
